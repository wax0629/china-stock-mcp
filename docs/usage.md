...
本文档面向最终使用者，介绍如何安装 `china-a-stock-mcp`、如何在主流 MCP 客户端（Claude Desktop / Cursor / Cline）中接入它，以及如何调用它提供的 10 个 Tool、3 个 Prompt 和 3 个 Resource。

如果你是开发者并希望了解内部架构、错误处理、缓存与限流细节，请参阅 [`docs/design.md`](design.md)。。
...
然后在你使用的 MCP 客户端里加一段配置，让它把 `china-a-stock-mcp` 拉起来。三个最常见的客户端示例：
...
`china-a-stock-mcp` 是标准的 MCP 服务器，**任何兼容 MCP 协议的客户端**都能接入，不限于 Claude Desktop。常见客户端见下表（按字母序）：
...
> Codex 目前**只支持 stdio 本地子进程**，不支持远程 MCP。`china-a-stock-mcp` 默认就是 stdio，无需额外改动。
...
```bash
# 直接通过 CLI 添加
codex mcp add china-stock -- uvx china-a-stock-mcp
...
A：必须同时满足：
1. 装了 `china-a-stock-mcp[tushare]` 或单独安装了 `tushare` 包。
...
3. 直接运行 `china-a-stock-mcp` 在终端中复现，stderr 会直接输出到屏幕。
...
- [`design.md`](design.md) — 内部架构、算法、Correctness Properties
